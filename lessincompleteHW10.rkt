...
;;; Part 6
;; Helper Functions
;; block-below? : block block -> bool
;; Takes in 2 blocks and check whether
;; The 2nd block is directly below the first block
(define (block-below? block1 block2)
  (cond
    [(and
      (>= (block-y block2) (block-y block1))
      (or
       (touch-target? block2
                      (make-posn (block-x block1) (+ 30 (block-y block1))))
       (touch-target? block2
                      (make-posn (+ 30 (block-x block1)) (+ 30 (block-y block1))))))
     #true]
    [else #false]))

(check-expect
 (block-below? (make-block 30 30 "lime") (make-block 100 100 "tomato")) #false)
(check-expect
 (block-below? (make-block 100 100 "lime") (make-block 100 130 "green")) #true)

;; any-below? : block list-of-blocks -> bool
;; Determines whether a there's at least one block
;; In the list that is directly below
(define (any-below? block1 blocks)
  (cond
    [(empty? blocks) #false]
    [(block-below? block1 (first blocks)) #true]
    [else (any-below? block1 (rest blocks))]))

(check-expect (any-below? (make-block 100 100 "lime") (list (make-block 100 0 "lime") (make-block 100 130 "lime"))) #true)
(check-expect (any-below? (make-block 100 100 "tomato") empty) #false)
...
(big-bang
  (make-game false (gen-blocks 10 600 300 "tomato"))
  [to-draw game-scene]
  [on-tick step-game]
  [on-mouse mouse-game])
